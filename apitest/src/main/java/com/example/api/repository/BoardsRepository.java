...
//  group by f.fno;
  ...
  @Query("select b, p, count(r.likes), count(distinct r) from Boards b " +
      "left outer join Photos p on p.boards = b " +
  ...
  // Native Query = SQL
  @Query(value = "select b.bno, p.pnum, p.photos_name, " +
      "count(r.likes), count(r.reviewsnum) " +
      "from db7.photos p left outer join db7.boards b on b.bno=p.boards_bno " +
      "left outer join db7.reviews r on b.bno=r.boards_bno " +
      "where p.pnum = " +
      "(select max(pnum) from db7.photos p2 where p2.boards_bno=b.bno) " +
  ...
  @Query("select b, p, count(r.likes), count(distinct r) from Boards b " +
      ...
      "where pnum = (select max(p2.pnum) from Photos p2 where p2.boards=b) " +
      "group by b ")
  Page<Object[]> getListPageImgJPQL(Pageable pageable);

  @Query("select boards, max(p.pnum) from Photos p group by boards")
  Page<Object[]> getMaxQuery(Pageable pageable);

  @Query("select b, p, count(r.likes), count(r) " +
      "from Boards b left outer join Photos p on p.boards=b " +
      "left outer join Reviews r on r.boards = b " +
