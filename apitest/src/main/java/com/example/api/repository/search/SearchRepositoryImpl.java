...
import com.example.api.entity.QPhotos;
...
//  }
  ...
  public Page<Object[]> searchPage(String type, String keyword, Pageable pageable) {
    ...
    jpqlQuery.leftJoin(photos).on(photos.boards.eq(boards));
    jpqlQuery.leftJoin(reviews).on(reviews.boards.eq(boards));

    //3) Tuple생성 : 조인을 한 결과의 데이터를 tuple로 생성
    JPQLQuery<Tuple> tuple = jpqlQuery.select(boards, photos, reviews.likes.count(),reviews.count());
