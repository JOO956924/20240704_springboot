...
//  group by m.mno;
  ...
  @Query("select mi.movie, max(mi.inum) from MovieImage mi group by mi.movie")
...
where m.mno=mi.movie_mno
group by m.mno order by m.mno desc;
-- inum은 최근값 출력, img_name은 개별속성이라서 정확히 출력 안됨.
...
and mi.inum = (select max(inum) from db7.movie_image mi2 where mi2.movie_mno=m.mno)
group by m.mno order by m.mno desc; -- review 카운트가 0이면 출력 안됨
