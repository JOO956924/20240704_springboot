...
//    jpqlQuery.leftJoin(photos).on(photos.feed.eq(feed));
//    jpqlQuery.leftJoin(review).on(review.feed.eq(feed));
//
//    JPQLQuery<Tuple> tuple = jpqlQuery.select(feed, photos.feed, review.count());
//    tuple.groupBy(feed);
//
//    log.info("==========================");
//    log.info(tuple);
...
//  }
  ...
  public Page<Object[]> searchPage(String type, String keyword, Pageable pageable) {
    ...
    JPQLQuery<Tuple> tuple = jpqlQuery.select(feed, photos, review.grade.avg().coalesce(0.0),review.count());

    //4) 조건절 생성
    BooleanBuilder booleanBuilder = new BooleanBuilder();
    BooleanExpression expression = feed.fno.gt(0L);
